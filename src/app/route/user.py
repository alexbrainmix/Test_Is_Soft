...
from app.crud import user, group
from app.models.models import UserDB, User
...
async def read_user(obj_id: int = Path(..., gt=0),):
    user_obj = await user.get_by_id(obj_id)
    if not user_obj:
...
async def update_user(obj: User, obj_id: int = Path(..., gt=0), ):
    group_obj = await group.get_by_id(obj.groups_id)
    if not group_obj:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Group not found")
    ...
    user_id = await user.update(obj_id, obj)
    response_object = {
        "id": user_id,
        "name": obj.name,
        "surname": obj.surname,
...
async def delete_user(obj_id: int = Path(..., gt=0)):
    user_obj = await user.get_by_id(obj_id)
    if not user_obj:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
